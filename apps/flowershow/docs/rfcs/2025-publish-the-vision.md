...
## Job stories (structured)

Instant publishing (overall vision): When I have a Markdown file, folder, image, or PDF on my machine, I want to publish it to a public (or private) URL in seconds, so that sharing it is faster than using Google Drive or similar tools.
...
### JS2 — Publish a single file (zero-context default)
...
When I publish a file or folder, I want it to be available almost immediately while processing continues in the background, so I get fast feedback even if deeper processing is still underway.
...
#### JS-NEW — Web UI publishing

When I do not want to use the CLI, I want to publish via a simple web UI with the same defaults, so I can use the workflow without local tooling.
