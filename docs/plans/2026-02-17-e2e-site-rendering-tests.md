...
const ROOT_DOMAIN = process.env.NEXT_PUBLIC_ROOT_DOMAIN || 'my.flowershow.local:3000';
...
test.describe('Links', () => {
  ...
  test('internal links resolve to site paths', async ({ page }) => {
    ...
    const syntaxLink = content.locator('a', { hasText: 'Internal link to basic syntax' });
    await expect(syntaxLink).toHaveAttribute('href', /\/@.+\/e2e-test-site\/basic-syntax$/);
  });
...
test.describe('Images', () => {
  ...
  test('image src points to asset via Next.js Image or direct URL', async ({ page }) => {
    const content = page.locator('#mdxpage');
    const img = content.locator('img').first();
...
test.describe('Code Blocks', () => {
  ...
  test('code blocks have language-specific syntax highlighting', async ({ page }) => {
    const content = page.locator('#mdxpage');
