# ADR 001: Replace Inngest with Cloudflare Workflows for the Publish Pipeline
...
**Anonymous path**: same as presigned paths but no `Publish` or `PublishFile` records are created; the Worker processes files but has no publish to credit. *(Bug: publish history is not tracked for anonymous publishes.)*
