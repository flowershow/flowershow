...
export function CodeWindow({ snippets }: { snippets: CodeSnippet[] }) {
  return (
    <Tab.Group
      defaultIndex={0}
      as="div"
      className="border-1 rounded-md border border-neutral-100 bg-neutral-50"
    >
      <Tab.List
      ...
      >
        ...
        {snippets.length > 1 &&
          snippets.map(({ file }, index) => (
            <Tab key={index} value={file}>
              {({ selected }) => (
                <div
                  className={clsx(
                    "relative flex h-10 shrink-0 items-center border-b border-r border-neutral-100 px-4 text-xs focus:outline-none",
                    selected
                      ? "border-b-0 bg-white"
      ...
      <Tab.Panels className="max-h-[600px] overflow-y-scroll">
        {snippets.map(({ code, language }, index) => (
          ...
          >
            <div className="w-full overflow-x-auto bg-white p-3 text-sm">
              <SyntaxHighlighter
