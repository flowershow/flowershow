...
export const DataPackageLayout: React.FC<Props> = ({ children, datapackage, gh_repository, gh_branch }) => {
    const { title, description, resources, views, created, updated, licenses, sources } = datapackage;
                                    ...
                                    {licenses.map((l) => {
                                        return (
                                            <a
                                                ...
                                                target="_blank"
                                                href={l.path}
                                                className="block mb-2 hover:text-[#6366F1]"
                                            >
                                                {l.title}
                                            </a>
                                        );
            ...
            <section className="my-12">
                ...
                <table className="table-auto divide-y divide-gray-300">
                    ...
                    <tbody>
                        {resources.map((r) => {
                            return (
                                <tr key={`resources-list-${r.name}`} className="even:bg-gray-50">
                                    <td>
                                        <a
                                            target="_blank"
                    ...
                    {resources.slice(0, 5).map((r) => {
                        return (
                            <div key={`resource-preview-${r.name}`} className="mt-10">
                                ...
                                {/* @ts-expect-error */}
                                <FlatUiTable url={`https://raw.githubusercontent.com/${gh_repository}/${gh_branch}/${r.path}`} />
                            </div>
